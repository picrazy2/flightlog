-- Actual flown distance (statute miles): length of the recorded GPS track including the
-- great-circle arcs filled over coverage gaps and the stitches to the dep/arr gates.
-- Summing haversine over [dep, ...track points, arr] yields exactly that (haversine
-- between two points = the great-circle bridge). Only set when a track exists; the UI
-- prefers it over route/great-circle in tracks mode.
alter table public.flights add column if not exists flown_distance_mi double precision;

-- Recreate the view so f.* exposes the new column.
