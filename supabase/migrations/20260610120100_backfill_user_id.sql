-- flights.user_id pre-existed as an unused uuid column (all null). We want a short
-- text id matching users.id ('alex') for /alex-style URLs, so replace it and
-- re-expose via the view. Drop the view first (it depends on the column).
drop view if exists public.v_flights_with_airports;

alter table public.flights drop column if exists user_id;
alter table public.flights add column user_id text not null default 'alex' references public.users(id);
