-- Airline-scheduled wheels times (AeroAPI scheduled_off / scheduled_on), so the
-- popup can show scheduled vs actual taxi and wheels times.
alter table public.flights
  add column if not exists provider_sched_takeoff timestamptz,
  add column if not exists provider_sched_landing timestamptz;

-- Recreate the view (atomic in a txn) so f.* exposes the new columns. Keep emails.
