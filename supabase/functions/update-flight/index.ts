...
export async function handleUpdateFlightRequest(
  ...
  try {
    ...
    const mergedRequest = mergeUpdateRequest(existingFlight, body);
    ...
    const { data: updatedFlight, error: loadError } = await supabase
    ...
    if (loadError) {
      throw new HttpError(
        500,
        `Failed to load updated flight: ${loadError.message}`,
      );
    }

    return jsonResponse<UpdateFlightResult>({
      ok: true,
      flight: updatedFlight as Record<string, unknown>,
      warnings,
    });
  } catch (error) {
...
async function resolveUpdatedBookingId(
  supabase: ReturnType<typeof createAdminClient>,
