...
export const MAP_CENTER_LAT = 30; // °N — frames more land in the northern hemisphere
...
// With the centre at 30°N this threshold is ≈ 1.21.
export const flatFitsViewport = (w: number, h: number) => w / h >= 1 / (2 * mercatorY(MAP_CENTER_LAT));
...
export function computeInitialView(w: number, h: number, ctx: StatContext): InitialView {
  if (flatFitsViewport(w, h)) {
    ...
    return { projection: "mercator", center: [0, MAP_CENTER_LAT], zoom: Math.max(0, Math.log2(w / 512)) };
  }
